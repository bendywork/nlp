...
class Tokenizer(object):
    ...
    def calc(self, sentence, DAG, route):
        ...
        for idx in xrange(N - 1, -1, -1):
            ...
            _tmp_list = []
            for x in DAG[idx]:
                # 获取单词片段的log(pwi)值 [idx,x]
                _tmp_idx_x = log(self.FREQ.get(sentence[idx:x + 1]) or 1) - logtotal
                # 从idx往后进行分割的概率 [idx,x] + [x+1,....]
                _tmp_p = _tmp_idx_x + route[x + 1][0]
                # 临时存储
                _tmp_list.append((_tmp_p, x))
            route[idx] = max(_tmp_list)  # 选择概率最大的可能
    ...
    def __cut_DAG(self, sentence):
        # 构建以某个字作为开头的时候，在词典中能够找到的词语组合列表
        DAG = self.get_DAG(sentence)
        # 基于DAG计算最有可能的概率以及分词方式
        route = {}
        self.calc(sentence, DAG, route)
        x = 0
        buf = ''
